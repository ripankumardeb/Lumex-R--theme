...
async function rebuildIconTheme(context) {
    ...
    for (const [ext, b64] of Object.entries(customIcons)) {
        ...
        if (defaults_1.SPECIAL_FILENAMES.includes(ext)) {
            fileNames[ext] = defKey;
        }
        else if (defaults_1.SPECIAL_FOLDERS.includes(ext)) {
    ...
    await vscode.workspace.getConfiguration().update('workbench.iconTheme', 'lumex-icons', vscode.ConfigurationTarget.Global);
}
// ─────────────────────────────────────────────
// Reset
// ─────────────────────────────────────────────
